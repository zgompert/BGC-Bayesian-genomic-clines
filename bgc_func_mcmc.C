...
void mcmcUpdate(paramcont * param, datacont * data, datadimcont * datadim, auxcont * auxvar,
  ...
  if (ngs == 1){ // update p0cnt and p1cnt
    updatePcnt(data,auxvar,datadim,data->p0cnt,data->glp0,datadim->nindP0,param->pi0Cur);
    updatePcnt(data,auxvar,datadim,data->p1cnt,data->glp1,datadim->nindP1,param->pi1Cur);
  }
  ...
  // draw hi from precomputed posterior stored as a histogram
  sampleHi(param,datadim);
...
/* control function for mcmc updates */
void gibbsHiUpdate(paramcont * param, datacont * data, datadimcont * datadim, auxcont * auxvar,
		   int allDiploid, int ngs, int refl){

  if(refl == 0){ // don't use reference set
  ...
  // set phi to hi
    setPhiH(param,datadim,datadim->nloci);
    ...
    // update hi, uses gibbs and saves
    updateHi(param,datadim,allDiploid);
  }
  // for reference locus set
  else{ 
  
    // update parental allele frequencies, pop 0 then pop 1
    if (ngs == 1){ // update p0cnt and p1cnt
      updatePcntRef(data,auxvar,datadim,data->refp0cnt,data->refglp0,datadim->nindP0,param->refpi0Cur);
      updatePcntRef(data,auxvar,datadim,data->refp1cnt,data->refglp1,datadim->nindP1,param->refpi1Cur);
    }
    
    updatePi(data->refp0cnt,param->refpi0Cur,datadim->nrefallele,datadim->nrefloci,auxvar->vecAllelen1,auxvar->vecAllelen2);
    updatePi(data->refp1cnt,param->refpi1Cur,datadim->nrefallele,datadim->nrefloci,auxvar->vecAllelen1,auxvar->vecAllelen2);
  // set phi to hi
    setPhiHRef(param,datadim,datadim->nrefloci);
    
    // update z, ancestry, uses gibbs sampling
    if (ngs == 0){
      cerr << "Cannot use reference set with non-ngs data, results unreliable" << endl;
    }
    else if (ngs == 1){ // ngs update of ancestry
       updateZngsRef(param,data,datadim,auxvar);
    }
    
    // update hi, uses gibbs and saves
    updateHiRef(param,datadim,allDiploid);
  }
...
void updateHi(paramcont * param, datadimcont * datadim, int allDiploid){
  int n, i;
  double a, b, h;
  int plcnt = 0;
  double prior = 0.5;
  ...
  for(n=0; n<datadim->nind; n++){
    ...
    if(allDiploid == 1){
      for(i=0; i<datadim->nloci; i++){
	a += gsl_matrix_int_get(param->zCur[i].mat, n, 0) +
	  gsl_matrix_int_get(param->zCur[i].mat, n, 1);
      }
      b = (2 * datadim->nloci) - a;
    }
    else{ // some loci could be haploid must check
      plcnt = 0;
      for(i=0; i<datadim->nloci; i++){
	if (gsl_vector_int_get(datadim->ploidyVec,i) == 2){ // diploid locus
	  a += gsl_matrix_int_get(param->zCur[i].mat, n, 0) +
	    gsl_matrix_int_get(param->zCur[i].mat, n, 1);
	  plcnt += 2;
	}
...
/* function to sample hi from the posterior using previous histogram */
void sampleHi(paramcont * param, datadimcont * datadim){
  int n;
  double h, u;
  ...
  for(n=0; n<datadim->nind; n++){
    ...
    h = gsl_histogram_pdf_sample(param->hihist[n].pdf, u);
    gsl_matrix_set(param->hi, n, 0, h);
  }
}   
...
//  b = 0.5 * v + gampriorb; //
//  if(trunc==0)// gamma sample
//	  *precision = gsl_ran_gamma(r, a, 1/b); // 1/b makes it a scale parameter for gsl
//  else{ //truncated gamma, solution from stack overflow
//	  u = gsl_ran_flat(r, 0, 1);
//	  x = gsl_cdf_gamma_P(trunc,a,1/b);
//          y = u * (-1 * gsl_cdf_gamma_P(trunc,a,1/b));
//         *precision = gsl_cdf_gamma_Pinv(x+y,a,1/b);
//
//  }
//  cerr << "sample precision:" << a << ", " << b << ", " << precision << endl;
//}
...
void updateTau(gsl_matrix * parameters, double * precision, int nloci, double trunc){
  ...
  *precision = -9; // set to -9 to enter loop
  for(i=0; i<nloci; i++){
    mn += gsl_matrix_get(parameters, i, 0);
  }
  mn = mn/nloci;

  for(i=0; i<nloci; i++){
    v += gsl_pow_2(gsl_matrix_get(parameters, i, 0) -mn);    
  }
  ...
  // truncated gamma
    ...
    x = gsl_cdf_gamma_P(trunc, a, 1/b);
    u = gsl_ran_flat(r, x, 1);
    *precision = gsl_cdf_gamma_Pinv(u, a, 1/b);
  }
  sd = sqrt(1/ *precision);
//  cout << a << ", " << b << ", " << v << " :: " << *precision << ", " << sd << endl;
...
void setPhiH(paramcont * param, datadimcont * datadim, int N){
  int i, n;
  double onePhi;
