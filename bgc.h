...
#define GEN 3 // number of genotypes for NGS data
extern gsl_rng * r;
...
struct datacont{
  ...
  // reference allele set
  gsl_matrix_int * refp0cnt;
  gsl_matrix_int * refp1cnt;
  mat_container_int * refphcnt;
  
  // old parental ngs containers
  mat_container_int * ngsp0cnt;
  mat_container_int * ngsp1cnt;

  // variables for ngs genotype likelihood format data
  mat_container * glp0; // one matrix per locus, then n ind rows by k = 3 genotypes
  mat_container * glp1;
  mat_container * glh;

  // varaibales for ngs genotype likelihood format data, reference loci for hindex only
  mat_container * refglp0; // one matrix per locus, then n ind rows by k = 3 genotypes
  mat_container * refglp1;
  mat_container * refglh;
...
struct paramcont{
  ...
  gsl_matrix * refpi0Cur; // parental allele frequences
  gsl_matrix * refpi1Cur; 
  gsl_matrix * refpi0Last; // parental allele frequences
  gsl_matrix * refpi1Last;
  
  mat_container_int * zCur; // ancestry
  mat_container_int * zLast; // ancestry
  
  mat_container_int * refzCur; // ancestry
  mat_container_int * refzLast; // ancestry
  ...
  hist_container * hihist; // container for hybrid index histograms
  
  gsl_matrix * phi; // ancestry probability

  gsl_matrix * refphi; // ancestry probability
...
void gibbsHiUpdate(paramcont * param, datacont * data, datadimcont * datadim, auxcont * auxvar,
		   int allDiploid, int ngs, int ref);
...
/* -------- functions for set of reference loci for h --------- */
