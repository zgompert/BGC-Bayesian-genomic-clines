// file: bgc_main.C
// Time-stamp: <Thursday, 17 June 2021, 14:45 MDT -- zgompert>
...
int main(int argc, char *argv[]) {
  ...
  int refl = 0; // use a different set of reference loci for hybrid index
  ...
  string refhybridfile = "undefined"; 
  string refp0file = "undefined";
  string refp1file = "undefined";
  ...
  while ((ch = getopt_long(argc, argv, "h:a:b:x:u:g:z:e:t:p:n:d:q:o:s:v:i:m:M:D:F:H:N:I:f:T:E:O:R:A:B:Z:",
  ...
  // for genotype likelihood format ngs data
  if (ngs == 1){
    getNgsDims(hybridfile,p0file,p1file,&data,&datadim);
    datadim.nallele = gsl_vector_int_calloc(datadim.nloci);
    gsl_vector_int_set_all(datadim.nallele, 2); // all bi-allelic
    datadim.maxNallele = 2;
    datadim.npop = 1;
    if(refl == 1){
      getRefDims(refhybridfile,&datadim);
      datadim.nrefallele = gsl_vector_int_calloc(datadim.nrefloci);
      gsl_vector_int_set_all(datadim.nrefallele,2);
    }
  }
  ...
  if(refl == 1){
    cerr << "Number of reference loci for hindex: " << datadim.nrefloci << endl;
  }
  if (linkMod == 1){
  ...
  if(refl == 1){
      data.refp0cnt = gsl_matrix_int_calloc(datadim.nrefloci,datadim.maxNallele);
      data.refp1cnt = gsl_matrix_int_calloc(datadim.nrefloci,datadim.maxNallele);
  }
  if (ngs == 1){
    data.glh = new mat_container[datadim.nloci];
    data.glp0 = new mat_container[datadim.nloci];
    data.glp1 = new mat_container[datadim.nloci];
    for (i=0; i<datadim.nloci; i++){
      data.glh[i].mat = gsl_matrix_calloc(datadim.nind,
					      GEN);
      data.glp0[i].mat = gsl_matrix_calloc(datadim.nindP0,
					      GEN);
      data.glp1[i].mat = gsl_matrix_calloc(datadim.nindP1,
					      GEN);
    }
    if(refl == 1){
      data.refglh = new mat_container[datadim.nrefloci];
      data.refglp0 = new mat_container[datadim.nrefloci];
      data.refglp1 = new mat_container[datadim.nrefloci];
      for (i=0; i<datadim.nrefloci; i++){
	data.refglh[i].mat = gsl_matrix_calloc(datadim.nind,
					    GEN);
	data.refglp0[i].mat = gsl_matrix_calloc(datadim.nindP0,
					     GEN);
	data.refglp1[i].mat = gsl_matrix_calloc(datadim.nindP1,
					     GEN);
      }
  ...
  if(refl==1){
    data.refphcnt = new mat_container_int[datadim.nrefloci];
    for (i=0; i<datadim.nrefloci; i++){
      data.refphcnt[i].mat = gsl_matrix_int_calloc(datadim.nind,2);// assumes diploid
    }
  }
  ...
  if(refl==1){
    param.refpi0Cur = gsl_matrix_calloc(datadim.nrefloci,datadim.maxNallele);
    param.refpi1Cur = gsl_matrix_calloc(datadim.nrefloci,datadim.maxNallele);
    param.refpi0Last = gsl_matrix_calloc(datadim.nrefloci,datadim.maxNallele);
    param.refpi1Last = gsl_matrix_calloc(datadim.nrefloci,datadim.maxNallele);
    param.refzCur = new mat_container_int[datadim.nrefloci];
    param.refzLast = new mat_container_int[datadim.nrefloci];
    for (i=0; i<datadim.nrefloci; i++){
      param.refzCur[i].mat = gsl_matrix_int_calloc(datadim.nind,PLOIDY);
      param.refzLast[i].mat = gsl_matrix_int_calloc(datadim.nind,PLOIDY);
    }
    param.refphi = gsl_matrix_calloc(datadim.nrefloci,datadim.nind);
  ...
  param.hihist = new hist_container[datadim.nind];
  for(i=0; i<datadim.nind; i++){
    param.hihist[i].hist = gsl_histogram_alloc(500); // allocate memery, 500 bins
    gsl_histogram_set_ranges_uniform(param.hihist[i].hist, 0.000001, 0.999999); // creates uniformly distrubted bins bounded by 0.000001 and 0.99999, avoids possible issues with h = 0 or 1 getting stuck
    param.hihist[i].pdf = gsl_histogram_pdf_alloc(500);
  }
  ...
  if (ngs == 0){
    ...
    if(refl == 1){
      getDataref(refhybridfile,refp0file,refp1file,&data,&datadim);
    }
  }
  ...
  // hybrid index MCMC loop
  cerr << "Generate posterior samples for hybrid index, this might take awhile" << endl;
  for(step=0; step<1000; step++)// hi burnin
    gibbsHiUpdate(&param,&data,&datadim,&auxvar,allDiploid,ngs,refl);
  for(i=0; i<datadim.nind; i++)// reset hist to remove burnin
    gsl_histogram_reset(param.hihist[i].hist);
  for(step=0; step<3000; step++)//hi sampling
    gibbsHiUpdate(&param,&data,&datadim,&auxvar,allDiploid,ngs,refl);

  // initialize hi histogram based on hi samples
  for(i=0; i<datadim.nind; i++){
    gsl_histogram_pdf_init(param.hihist[i].pdf, param.hihist[i].hist);
  }
  
  // main MCMC loop
  cerr << "Beginning main MCMC analyses" << endl;
